...
export class UserIdHelper {
  ...
  /**
   * Generate a unique auto-incrementing employee ID in the format: EMP-XXX
   * Where XXX is a zero-padded sequential number (e.g., EMP-001, EMP-002)
   */
  static async generateEmployeeId(
    ...
    if (users.length > 0 && users[0].employeeId) {
      // Extract number from existing employee ID (e.g., "EMP-001" -> 1)
      const match = users[0].employeeId.match(/^EMP-(\d+)$/);
      if (match) {
    ...
    // Fallback: use timestamp-based serial if we can't find a unique one
    const timestampSerial = String(Date.now()).slice(-6);
    return `EMP-${timestampSerial}`;
  }
}
